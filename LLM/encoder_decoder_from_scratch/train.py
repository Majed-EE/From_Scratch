...
def greedy_decode(model,source,source_mask,tokenizer_src,tokenizer_tgt,max_len,device):
    ...
    # precompute the encoer output and rewuse it for every token we get from the decoder
    encoder_output=model.encode(source,source_mask)
    #initialize the decoder input wiht the sos token
    decoder_input=torch.tensor(1,1).fill_(sos_idx).type_as(source).to(device)

    while True:
        if decoder_input.size(1)==max_len:
            break

        # build mask for the target (decoder inpyt)
...
def run_validation(model,validation_ds, tokenizer_src,tokenizer_tgt, max_len, device, print_msg, global_state,writer,num_examples=2):
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            assert encoder_input.size(0)==1, "Batc size must be 1 for validation"
            ...
            if count==num_examples:
                break
    if writer:
        # torchmetrics, charErrorRatem BLEU, WordErrorRate
        pass
