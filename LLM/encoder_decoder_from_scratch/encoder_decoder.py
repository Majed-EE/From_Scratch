...
class MultiHeadAttention(nn.Module):
    ...
    def attention(q,k,v,mask,dropout: nn.Dropout):
        d_k=q.shape[-1] # d_k=embd_dim --> d_model
    ...
    def forward(self,q,k,v):
        ...
        x,self.attention_scores = MultiHeadAttention.attention(q, k, v, None, self.dropout) # why is it called x ?
...
class ResidualConnection(nn.Module):
    ...
    def forward(self, x, sublayer):
        return x + self.dropout(sublayer(self.norm(x))) #?? what is sublayer
...
class ProjectionLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.log_softmax(self.proj(x), dim=-1)  # difference between log_softmax and softmax
